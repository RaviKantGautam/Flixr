...
function table(output, find) {
    ...
    if (find == 'pending') {
        var t = "";
        for (var i of output) {
            t += '<tr><td>' + i.srno + '</td><td>' + i.orderno + '</td><td>' + i.datatime + '</td><td>' + i.grandtotal + '</td><td>' + i.paymentmethod + '</td><td><div>' + i.useremail + '</div><div>' + i.userphone + '</div></td>';
            t += "<td><a class='btn btn-outline-secondary' onclick='showdetailmodal(" + JSON.stringify(i.orderno) + ")'>View Details</a></td>";
            t += "<td><button class='btn btn-outline-primary' onclick='showmodal(" + JSON.stringify(i.orderno) + "," + JSON.stringify(find) + "," + JSON.stringify(i.phone) + ")'>Ship Order</button></td></tr>";
        }
        document.getElementById('tfilter').innerHTML = t;
    } else if (find == 'Shipped') {
        var t = "";
        for (var i of output) {
            t += '<tr><td>' + i.srno + '</td><td>' + i.orderno + '</td><td>' + i.datatime + '</td><td>' + i.grandtotal + '</td><td>' + i.paymentmethod + '</td><td><div>' + i.useremail + '</div><div>' + i.userphone + '</div></td>';
            t += "<td><a class='btn btn-outline-secondary' onclick='showdetailmodal(" + JSON.stringify(i.orderno) + ")'>View Details</a></td>";
            t += "<td><button class='btn btn-outline-primary' onclick='showmodal(" + JSON.stringify(i.orderno) + "," + JSON.stringify(find) + "," + JSON.stringify(i.phone) + ")'>Dispatch Order</button></td></tr>";
        }
        document.getElementById('tfilter').innerHTML = t;
...
function showmodal(orderno, find, phone) {
    if (find == 'pending') {
        $('#orderno').val(orderno);
        // $('#email').val(email);
        $("#shipModal").modal('show');
    } else {
        $('#disorderno').val(orderno);
        // $('#disphone').val(phone);
